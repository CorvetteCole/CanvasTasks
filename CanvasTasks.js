...
(function dartProgram() {
  ...
  F = {
    main: function() {
      var t1, t2, testID, _i, tasklist, t3, i, newTask;
      for (t1 = J.get$items$x(Tasks.Tasklists.list()), t2 = t1.length, testID = null, _i = 0; _i < t1.length; t1.length === t2 || (0, H.throwConcurrentModificationError)(t1), ++_i) {
        tasklist = t1[_i];
        t3 = J.getInterceptor$x(tasklist);
        if (J.$eq$(t3.get$title(tasklist), "test"))
          testID = t3.get$id(tasklist);
      }
      for (i = 0; i < 20; ++i) {
        newTask = B.Task$(null);
        C.Task_methods.set$title(newTask, "test " + i);
        C.Task_methods.set$notes(newTask, "this is a test of inserting tasks");
        Tasks.Tasks.insert(newTask, testID);
      }
    }
  };
  var holders = [C, H, J, P, W, B, T, F];
  ...
  (function inheritance() {
    ...
    _inheritMany(J.JavaScriptObject, [J.PlainJavaScriptObject, J.UnknownJavaScriptObject, J.JavaScriptFunction, B.Tasklists, B.Tasklist, B.Tasks, B.Task, B.TaskLinks, T.DriveApp, T.File, T.Folder]);
